...
export class CelIndex {
    ...
    public set(p: number | CelIndex, y?: number) {
        if (typeof p === 'number') {
            this.row = p;
            this.col = y;
        } else {
...
export class DrawData {
     ...
     */
    public init(width: number, height: number, data?: ArrayBuffer): void {
     ...
     */
    public getColorCount(r: number, g: number, b: number, a: number = 255): Object {
    ...
    /**上一次绘制的直线的宽度 */
    private previousLineWidth: number = 2;
     ...
     * @param b 线段端点是否为圆形
     */
    public setLineCircleEnd(b: boolean): void {
     ...
     * @param y3    顶点3坐标Y
     */
    public triangle(x1: number, y1: number, x2: number, y2: number, x3: number, y3: number): void {
        let pList: Vec2[] = [];
